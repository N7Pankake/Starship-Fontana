...
void SFAsset::AlienMovementNorth() {
  int w, h;
  SDL_GetRendererOutputSize(sf_window->getRenderer(), &w, &h);
...
void SFAsset::AlienMovementSouth() {
  int w, h;
  SDL_GetRendererOutputSize(sf_window->getRenderer(), &w, &h);
...
void SFAsset::HandleCollision() {
  if(SFASSET_PROJECTILE == type || SFASSET_ALIENPROJECTILE == type || SFASSET_ALIEN == type || SFASSET_WALL == type || SFASSET_COIN == type || SFASSET_PLAYER == type ) {
    SetNotAlive();
  }
}
